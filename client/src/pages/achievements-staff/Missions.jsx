...
export default function Missions() {
  ...
  const getMissions = () => {
    http.get("/mission").then((res) => {
      setMissionList(res.data);
    });
  };

  const searchMissions = () => {
    http.get(`/mission?search=${search}`).then((res) => {
      setMissionList(res.data);
    });
  };
  ...
  const deleteMission = () => {
    http.delete(`/mission/${idToDelete}`).then((res) => {
        ...
        <div class="w-full mx-auto p-2 text-gray-800 dark:text-seashell/90 relative overflow-hidden min-w-full max-w-3xl">
          <div className="flex space-x-4 align-items-center justify-center">
            ...
            <div className="pt-3.5">
              <Link to="/staff/achievements/AddMissions">
                ...
                <table class="min-w-full text-sm text-gray-400 divide-y-2 divide-ultraViolet dark:divide-thistle">
                  <thead class="bg-orange-100 dark:bg-black dark:bg-opacity-20 text-sm uppercase font-medium">
                    <tr>
                      ...
                      <th class="px-10 py-3 text-left tracking-wider"> Exp</th>
                  ...
                  <tbody class="divide-y divide-thistle dark:divide-fedora">
                    {missionList.map((mission, id) => (
                      <tr
                        ...
                        onClick={() => {
                          navigate(
                            `/staff/achievements/EditMissions/${mission.missionId}`
                          );
                        }}
                      >
                        <td class="pl-4">{mission.missionId}</td>
                        ...
                        <td class="px-8 py-2 whitespace-nowrap">
                          <div
                            onClick={(event) => {
                              ...
                              setIdToDelete(mission.missionId);
                              setNameToDelete(mission.name);
                              setOpen(true);
                            }}
