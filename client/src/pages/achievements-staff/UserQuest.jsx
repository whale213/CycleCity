...
export default function UserQuest() {
  ...
  const getUserQuest = () => {
    http.get("/userQuest").then((res) => {
      setuserQuestList(res.data);
    });
  };

  const searchUserQuest = () => {
    http.get(`/userQuest?search=${search}`).then((res) => {
      setuserQuestList(res.data);
    });
  };
  ...
  const deleteQuest = () => {
    http.delete(`/userQuest/${idToDelete}`).then((res) => {
        ...
        <div className="w-full mx-auto p-2 text-gray-800 dark:text-seashell/90 relative overflow-hidden min-w-full max-w-3xl">
          <div className="flex space-x-4 align-items-center justify-center">
            ...
            <div className="pt-3.5">
              <Link to="/staff/achievements/AddUserQuest">
              ...
              <div className="shadow overflow-auto sm:rounded-lg">
                <table className="min-w-full text-sm text-gray-400 divide-y-2 divide-ultraViolet dark:divide-thistle">
                  ...
                  <tbody className="divide-y divide-thistle dark:divide-fedora">
                    {userQuestList.map((userQuest, id) => (
                      <tr
                        className="hover:bg-orange-100 dark:hover:bg-onyx group cursor-pointer"
                        key={id}
                        onClick={() => {
                          navigate(
                            `/staff/achievements/EditUserQuest/${userQuest.userQuestId}`
                          );
                        }}
                      >
                        <td className="pl-4">{userQuest.userQuestId}</td>
                        ...
                        <td className="px-8 py-2 whitespace-nowrap">
                          <div
                            onClick={(event) => {
                              ...
                              setIdToDelete(userQuest.userQuestId);
                              setNameToDelete(userQuest.quest.name);
                              setOpen(true);
                            }}
